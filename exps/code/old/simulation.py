...
def fitGenSigmoid(xObs,yObs):
    ...
    while changed:
        ...
        # update K
        Knew = K - gamma*updateK(A,K,Q,B,M,v,xObs,yObs)
...
def testChao92(entryList,qS,tQ,testQ):
    ...
    #Fit polynomial    
    xObsList = [0.0]
    xObsList.extend(unique[:testQ-1])
    yObsList = unique[:testQ]
            
    xObs = np.array(xObsList)
    yObs = np.array(yObsList)    
    ...
    printUniquePlotsLog(unique, fitLog, plotTitle,tQ,xObsList)
    printUniquePlotsPoly(unique, fitPoly, plotTitle,tQ,xObsList)
...
def printUniquePlotsLog(unique, fitLog, title,tQ,xObs):
    ...
    # Tweak spacing to prevent clipping of ylabel
    plt.subplots_adjust(left=0.15)
    fileName = '/Users/thodoris/Documents/ThodPh.D./Papers/YelpSimulation/figs/fit/reviews_log_test/' + title + '.pdf'
    #plt.show()
    plt.savefig(fileName)
    plt.close()

def printUniquePlotsPoly(unique, fitPoly, title,tQ,xObs):
