...
def trainPredictAr(observedData):
...
def testChao92(entryList,qS,tQ,testQ):
    ...
    for i in range(tQ):
        ...
        print 'Actual unique objects = ',sampleUniqueNew
        ...
        xObs.append(i+2)
        if i > testQ:
            pred = trainPredictAr(unique)
            arPredictions.append(pred)
            print '\nPrediction for next step:', pred
    
    #Fit polynomial                
    xObsList = xObs[:testQ]
    yObsList = unique[:testQ]
    xObsArray = np.array(xObsList)
    yObsArray = np.array(yObsList)    
            
    z = np.polyfit(xObsArray,yObsArray,3)
    p = np.poly1d(z)            
    fitPoly = []
    ...
    xValues = range(testQ,tQ+1)
    for x in xValues:        
        fitPoly.append(p(x))
        
        xObsList.append(xObs[x])
        yObsList.append(unique[x])
    ...
    print len(unique[testQ+2:])
    print len(fitPoly[testQ+2:])
    print len(arPredictions)
    print len(xObs[testQ+1:tQ])
    ...
    #printUniquePlots(unique[testQ+2:], fitPoly[testQ+2:], arPredictions, plotTitle,tQ,xObsList[testQ+1:tQ])
...
def printUniquePlots(unique, fitPoly, arPredictions, title,tQ,xObs):
    ...
    #x = np.linspace(1.0, tQ+1, tQ+1)
    x = np.array(xObs)
    yUnique = np.array(unique)
    yUniqueFitPoly = np.array(fitPoly)
    yUniqueAr = np.array(arPredictions)
    
    plt.plot(x, yUnique, 'k', x, yUniqueFitPoly, 'k:', x, yUniqueAr, 'k--')
    plt.legend(('Unique', 'UniqueFitPoly', 'UniqueARMA'),
               'upper left', shadow=True)
    plt.title(title, fontdict=font)
    plt.xlabel('# queries', fontdict=font)
    plt.ylabel('# items', fontdict=font)
