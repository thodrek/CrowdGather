...
def testChao92(entryList,qS,tQ,testQ):
    ...
    xValues = np.array(range(testQ+1,tQ+2))        
    for x in xValues:        
        fitLog.append(genSigmoid(A,K,Q,B,M,v,x))
        fitPoly.append(p(x))
        
        xObsList.append(x)
        yObsList.append(unique[x-1])
    ...
    printUniquePlotsLog(unique, fitLog, plotTitle,tQ)
    printUniquePlotsPoly(unique, fitPoly, plotTitle,tQ)
...
def printUniquePlotsLog(unique, fitLog, title,tQ):
...
def printUniquePlotsPoly(unique, fitPoly, title,tQ):
